...
export function BlockEnd({position = [0, 0, 0]}){
    const hambureger = useGLTF('./hamburger.glb')

    hambureger.scene.children.forEach((mesh)=>
    {
    ...
    return <group position = {position}>
        ...
        <RigidBody type ="fixed" colliders="hull" position={[0, 0.25, 0]} restitution={0.2} friction={0}>
            <primitive castShadow object={hambureger.scene} scale ={0.2}/>
        </RigidBody>
    </group>
}
...
export function BlockSpinner({position =[0, 0, 0]})
{
    ...
    // 이 방법은 매우 느려질 수 있어서 0.2보다 느려지지 않도록
    const [speed] = useState(()=>(Math.random()+0.2)*(Math.random() <0.5?-1:1))

    useFrame((state) =>
    {
        const time = state.clock.getElapsedTime()
        // 동일 : const time = state.clock.getElapsedTime
